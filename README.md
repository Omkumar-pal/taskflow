...
**Live app:** https://taskflow-nu-ten.vercel.app
**Live API:** https://taskflow-backend-azrg.onrender.com

> Both are on free hosting tiers. The backend sleeps after 15 minutes of inactivity (first request after that takes ~30–60s to wake up), and the SQLite database resets to the seeded demo data on every redeploy or wake cycle — this is expected, not a bug. See [Persistence on the free tier](#persistence-on-the-free-tier) below.
...
See [`backend/schema.sql`](backend/schema.sql) for the full definitions. Summary:
...
Foreign keys cascade on delete (deleting a board removes its columns and tasks). SQLite has foreign keys off by default per-connection, so `db.js` explicitly runs `PRAGMA foreign_keys = ON` — without this, `ON DELETE CASCADE` would silently do nothing.
...
All error responses are `{ "error": "..." }` with an appropriate 4xx/5xx status. The backend validates inputs independently of the frontend (e.g. rejects empty titles, missing foreign keys, invalid priority values).
...
Open http://localhost:5173 — the board loads, backed by the seeded SQLite data at `backend/taskflow.db`.
...
Covers: rejecting an empty title, moving a task updates `column_id`, and two tests calling the DB-layer query functions directly (`getTasksByPriority`, `getTaskCountsPerColumn`) against known seed data.
...
Render's free web services have an **ephemeral filesystem** — any local file (including a SQLite `.db` file) is reset whenever the service redeploys _or_ spins down from inactivity and wakes back up. There's no code bug here; it's a hosting-tier tradeoff.
